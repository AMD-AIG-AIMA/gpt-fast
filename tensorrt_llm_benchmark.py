...
import numpy as np
import torch
from tensorrt_llm_utils import (DEFAULT_HF_MODEL_DIRS, DEFAULT_PROMPT_TEMPLATES,
...
def sample_requests(
    ...
    for data in dataset:
        ...
        # Get the first turn as the prompt
        prompt = data["turns"][0]
        prompt_token_ids = tokenizer(prompt).input_ids
        prompt_len = len(prompt_token_ids)

        # Use fixed_output_len if provided, otherwise use a default value
        output_len = fixed_output_len if fixed_output_len is not None else 100
...
def parse_arguments(args=None):
    ...
    parser.add_argument('--dataset', type=str, help='Path to the dataset file containing text prompts')
    parser.add_argument('--num_prompts', type=int, default=1000, help='Number of prompts to process')
    parser.add_argument('--output_len', type=int, help='Fixed output length for each request')
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size for processing requests')
    parser.add_argument('--output_json', type=str, help='Path to save the throughput results in JSON format')
...
def parse_input(tokenizer,
    ...
    batch_input_ids = []
    if input_file is None and dataset is None:
        ...
        batch_input_ids = sample_requests(dataset, args.num_prompts, tokenizer, args.output_len)
...
def print_output(tokenizer,
                 ...
                 output_walltime: Optional[float] = None,
                 output_speed: Optional[float] = None,
                 accept_length: Optional[int] = None):
    num_output_sents, num_beams, _ = output_ids.size()
    batch_size = len(input_lengths)
    num_return_sequences = num_output_sents // batch_size

    if output_csv is None and output_npy is None and output_json is None:
    ...
    if output_json is not None:
        ...
        for i, output in enumerate(outputs):
            output = output_ids[i]
            input_text = tokenizer.decode(output[i][0][:input_lengths[i]].tolist())
            # In this case, we only have one beam
            output_text = tokenizer.decode(output[i][0][input_lengths[i]:sequence_lengths[i][0]].tolist())
            results.append({
                'question_id':  len(results),
                'turns': [input_text, output_text],
                'tokens_generated': sequence_lengths[i][0],
                'walltime': output_walltime[i],
                'speed': output_speed[i],
                'accept_counts': accept_length[i] if accept_length is not None else None,
            })
...
def main(args):
    ...
    def batch_generator(batch_input_ids, batch_size):
        if batch_size is None:
            yield batch_input_ids
        else:
            for i in range(0, len(batch_input_ids), batch_size):
                yield batch_input_ids[i:i+batch_size]
    outputs = []
    speeds = []
    if args.draft_target_model_config is None:  # Normal run except Draft-Target-Model
        ...
        if not args.use_py_session:
            runner_kwargs.update(
                max_batch_size=len(batch_input_ids) if args.batch_size is None else args.batch_size,
                max_input_len=max(
        ...
        with torch.no_grad():
            for batch_subset in batch_generator(batch_input_ids, args.batch_size):
                with TimeProfiler(name='generate'):
                    output = runner.generate(
                        batch_input_ids=decoder_input_ids if is_enc_dec else batch_subset,
        ...
        for batch_subset in batch_generator(batch_input_ids, args.batch_size):
            with TimeProfiler(name='generate'):
                output = run_draft_target_model(batch_subset, args, runtime_rank,
                                                end_id, pad_id, stop_words_list,
                                                bad_words_list, tokenizer.vocab_size)
            outputs.append(list(output)[0])
        output, runner = list(output)[0]
    
    if runtime_rank == 0:
        if args.batch_size is None:
            output_ids = outputs[0]['output_ids']
            sequence_lengths = outputs[0]['sequence_lengths']
        else:
            output_ids = [o['output_ids'] for o in outputs]
            sequence_lengths = [o['sequence_lengths'] for o in outputs]
        speeds = [s[0] / t for s, t in zip(sequence_lengths, TimeProfiler._timings['generate'])]
        context_logits = None
        ...
        print_output(tokenizer,
                        ...
                        output_walltime=TimeProfiler._timings['generate'],
                        output_speed=speeds,)
        mean_speed = sum(speeds) / len(speeds)
        max_speed = max(speeds)
        elapsed_time = sum(TimeProfiler._timings['generate'])
        overall_speed = sum(s[0] for s in sequence_lengths) / elapsed_time
